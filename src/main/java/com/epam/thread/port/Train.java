...
public class Train extends TimerTask {
  ...
  private static final double TRAIN_CAPACITY = .2 * Port.MAX_CAPACITY.get();
  @Override
  public void run() {
     if(Port.currentNumberContainers.get() > LOAD_FACTOR * Port.MAX_CAPACITY.get()) {
       Port.currentNumberContainers.addAndGet(-1 * (int)TRAIN_CAPACITY);
     }
    if(Port.currentNumberContainers.get() < (1 - LOAD_FACTOR) * Port.MAX_CAPACITY.get()) {
